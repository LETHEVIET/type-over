...
export class TypeOverSession {
  ...
  init(): boolean {
    ...
    document.addEventListener(
      'beforeinput',
      this.beforeInput as any,
      { capture: true, signal: this.abortController.signal } as AddEventListenerOptions,
    );
  ...
  onSelectionChange = () => {
    ...
    if (!anchor) return;
    const frag = (anchor instanceof Element ? anchor : anchor.parentElement)?.closest(
      '.typeover-fragment',
    );
    if (!frag) return;
    const container = sel.anchorNode as Node;
    let el: HTMLElement | null =
      container instanceof Element ? (container as HTMLElement) : container.parentElement;
    if (!el) return;
  ...
  placeCaret(i: number) {
    ...
    if (i <= 0) {
      const first = list[0];
      const tn = first.text;
      tn ? range.setStart(tn, 0) : range.setStartBefore(first.el);
    } else if (i >= list.length) {
      const last = list[list.length - 1];
      const tn = last.text;
      tn ? range.setStart(tn, tn.length) : range.setStartAfter(last.el);
    } else {
      const prev = list[i - 1];
      const tn = prev.text;
      tn ? range.setStart(tn, tn.length) : range.setStartAfter(prev.el);
    }
    range.collapse(true);
    sel.addRange(range);
